...
public class UCS extends TreeSearch {
    ...
    public boolean solve() {
        // Création d'une file de priorité pour gérer les nœuds à explorer
        ...
        while (!frontier.isEmpty()) {
            // Récupère le nœud avec le coût le plus faible
            SearchNode node = frontier.poll();
            State state = node.getState();

            if (ArgParse.DEBUG) {
                System.out.println("Exploring node with state: " + state + ", cost: " + node.getCost());
            }
            ...
            for (Action action : actions) {
                ...
                // Si le nœud enfant n'est pas dans la liste explorée ou la frontière
